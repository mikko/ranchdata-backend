...
const routes = [
    ...
    {
        method: 'GET',
        path: `/${apiPrefix}/journalentries/latest/{entryCount}`,
        config: {
            auth: 'session',
            handler: (request, reply) => {
                return Journal.getLatestEntries(request.auth.credentials.userId, request.params.entryCount)
                    .then(entries => {
                        reply(entries);
                    })
                    .catch(err => {
    ...
    },
    {
        ...
        config: {
            ...
            handler: (request, reply) => {
                ...
                const payload = request.payload;
                const userId = request.auth.credentials.userId;
                View.createView(userId, payload.title, payload.viewData);
                reply();
            },
            validate: {
                payload: {
